...
- **The Sparse Gatekeeper**: Khusus untuk mengakali keruntuhan dimensi pada prediksi *Zero-Shot* murni, sebuah kendali leksikal dipasang. Jika teks berukuran kecil ($< 50$ karakter) dan memiliki $0.0$ irisan kata dengan taksonomi label (*Jaccard 0%*), skor Dense Vector akan dihanguskan ke $0.0$. Penjelasan arsitektur ini dicatat di `_fondasi/dimensional_collapse_stki.md`.
