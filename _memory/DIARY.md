...
- **Analisis Kritis & Bug Ditemukan**: Secara menakjubkan, struktur Hybrid STKI (Dense + Sparse) **berhasil menahan seluruh anomali STKI** (skor BM25 Penalty menyelamatkan cacat bawaan model *Deep Learning* IndoBERT pada kata bahasa Inggris). Namun, di modul DS Pilar 3, ditemukan **1 Logic Flaw Kritis**: Algoritma Fuzzy Alignment secara brutal mengambil kolom "Agama" (yang kebetulan berisi A/B) dan menjadikannya kolom Nilai IPK Akademik mahasiswa.
- **Penyelesaian Mutlak**: Modul `TKI/pilar3_transformer.py` telah saya *patch* dengan *Negative Constraints* (Penolakan terhadap kolom yang terindikasi entitas identitas, agama, golongan darah, dan gender). *Future Warning* Pandas `include_groups=False` pada perhitungan agregat nilai SKS juga telah diberantas. Keseluruhan audit didokumentasikan di `testing/analisis_10_fundamental.md`.
