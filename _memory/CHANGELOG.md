...
### Fixed
- Pencegahan Anomali *Dimensional Collapse*: Menyelesaikan kasus teoretis di mana teks OOV seperti "halo halo badnung" mendapatkan Cosine Similarity 99% dengan label klasifikasi. Ini disebabkan oleh ruang keluaran model yang hanya berdimensi 5 (keterbatasan derajat kebebasan), yang memaksa teks *nonsense* meluruh ke vektor bias yang sama persis dengan label OOV.
- Implementasi *The Sparse Gatekeeper* (Lexical Penalty): Menambahkan *hard-cutoff* heuristik di `app_web.py` pada *pipeline* `/api/predict` (Skenario Zero-Shot). Jika dokumen berukuran $< 50$ karakter dan rasio interseksi Jaccard (Lexical Overlap) dengan nama kelas adalah $0.0$, skor *Dense Similarity* akan dipangkas mutlak menjadi $0.0$.
